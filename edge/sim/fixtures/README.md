...
| `transport` | ✓ | `mllp` \| `serial-rs232` \| `file` \| `loopback` — the real wire; the harness replays via a matching `Transport` (loopback today). |
...
| `message.framing` | ✓ | `raw` \| `mllp` \| `astm` — framing the transport applies/strips. |

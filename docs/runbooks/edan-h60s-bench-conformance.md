...
| `MSH-16` on data | `1=QC` (H60 doc) | `0` on patient-mode results, `2` on the connection-test ping, `3` on host-query QRY^R02 (20260707-H60-7907) — all three match the **H90-series** protocol map (`EDAN\WI\82-01.54.460907` §3.2.1: 0=sample, 1=blood-cell QC, 2=test connection, 3=obtain info/host-query, 4=protein control, 1000=production tooling); the H60-specific doc lists only 0/1, and the bench proves the physical H60S follows the H90 doc. **LIS-110 shipped the vendor-aware profile** (bridge `HL7ResultParser.fromEdanMsh16` + sim `oru.py _result_type`): `0`/empty→patient, `1`→QC (QC OBR layout 460907 §3.2.3: PID-less, lot=OBR-13, level=OBR-3), every other value — the documented non-result `2`/`3`, the undispositioned `4` (LIS-224) / `1000`, and unknowns — →**QC hold, never patient** (the observed `2`/`3` frames are MSH-only and yield no results). **Residual deviation per LIS-110 AC-5:** `1=QC` and the §3.2.3 QC layout are protocol-documented but not yet wire-confirmed — no QC-mode capture exists. A future QC-mode bench run must confirm them or amend the profile |

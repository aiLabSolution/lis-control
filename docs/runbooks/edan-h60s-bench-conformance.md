...
| `MSH-16` on data | `1=QC` (KB) | `0` on patient-mode results, `2` on the connection-test ping, `3` on host-query QRY^R02 (20260707-H60-7907) — the KB documents only `0`/`1`. **LIS-110 shipped the vendor-aware profile** (bridge `HL7ResultParser.fromEdanMsh16` + sim `oru.py _result_type`): `0`/empty→patient, `1`→QC per KB §3.2.1 (QC OBR layout §3.2.3: PID-less, lot=OBR-13, level=OBR-3), any other value→**fail-closed QC hold** (never patient; the observed `2`/`3` frames are MSH-only and yield no results). **Deviation recorded per LIS-110 AC-5:** `1=QC` is manual-derived, not yet wire-confirmed — no QC-mode capture exists. A future QC-mode bench run must confirm MSH-16=1 + the §3.2.3 OBR layout, or amend the profile |

...
Every wire fact below is bench-verified against analyzer SN `010101003401230113`
...
The bench established that the X3 **opens one long-lived connection and reuses it for
every transaction, indefinitely** — we observed two operator upload actions ~90 seconds
apart sharing a single connection. At the default `10`, the bridge tears that channel
down after 10s of idle, over and over.
...
1. **`name` is cosmetic — do not rely on it.** AC6 proved the X3 enforces *no* identity
   matching: with `Host ID` set to `NOTLIS`, it stamped `NOTLIS` into `H-5` and delivered
   the batch anyway, LIS green throughout. Both `Analyzer ID` (`H-3`) and `Host ID`
   (`H-5`) are operator-editable free text with zero validation. Never route or validate
   on them.
...
| **One long-lived connection, reused for every transaction** | Never close on `<EOT>`; never close on idle (Step 2) |

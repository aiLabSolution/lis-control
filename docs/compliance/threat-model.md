...
- **v1 fleet scope (open decision #4)** — ✅ **RESOLVED by [ADR-0008](../adr/0008-interface-engine-stack-and-fleet-scope.md):** minimal **HL7-v2.x/MLLP-first** v1 fleet — **pinned 2026-06-27 (Seamaty SD1 added 2026-06-29):** anchor **EDAN H60S** (HL7 V2.4/MLLP) + H99S + RAYTO RT-7600 + **Seamaty SD1** (HL7 v2.3.1/MLLP, upload-only), result-ingestion first; **MAGLUMI X3** (ASTM E1394-97/TCP) = v1.1 — *amended 2026-07-06 (LIS-178 / ADR-0008 amendment):* the X3 attaches **natively, direct to the bridge's ASTM listener** (built-in LIS interface; HL7 v2.5 documented alternative); the SnibeLis middleware is dropped from the topology, so the X3's former REQ-PRIV-09 DPA flow-down dissolves — **one fewer PHI-touching sub-processor**, and its TB-1 boundary is an ordinary direct-attach analyzer boundary like the rest of the fleet. The **serial group (ERBA EC90), the incoming HETO AU120, and bidirectional host-query are deferred post-pilot under change control (REQ-QMS-03)** — keeping the pilot TB-1/TB-2 surface and REQ-CONF-01 burden minimal. (SD1 adds one HL7/MLLP TB-1 in the same protocol family as the H60S anchor — low marginal attack surface — plus one REQ-CONF-01 bench report.)

...
> **Stage-0 SCAFFOLD / BASELINE THREAT MODEL — prepared for LIS-10 (S0.8), drafted by an agent, pending human review. Date: 2026-06-23.**
> This is the first-pass STRIDE threat model over the LabSolution LIS reference architecture. It establishes the security baseline that every later stage refines and that the **Stage-5 penetration test (REQ-SEC-04)** must exercise. It is NOT a completed security assessment: items needing a key-management decision, a deployment-topology decision, a PKI design, or a signed pen-test scope are marked **[NEEDS-HUMAN]**. Cross-referenced against the canonical requirement-ID seed / traceability matrix and the Stage-0 sibling issues (LIS-3 … LIS-9).
...
**In scope (Stage-0 baseline).** The full reference data-flow: instrument edge (HL7 v2.x over MLLP / ASTM-family LIS serial / file-watch) → LabSolution-owned interface engine → normalization (LOINC/UCUM) → OpenELIS core + PostgreSQL → FHIR R4 (HAPI FHIR) / HL7 v2 outbound → EMR/HIS/PhilHealth; plus site↔central offline sync (store-and-forward + replication); plus the cross-cutting RBAC + append-only audit + encryption controls.
...
**Topology caveat (load-bearing).** The threat surface is materially shaped by **open leadership decision #3 — deployment topology** (central-cloud + thin sites vs. full on-prem per site + central sync). This baseline analyzes the **on-prem-per-site + central-sync** case as the worst-case attack surface (more PHI at rest in more physical locations, more sync links, more edge appliances to lose), and flags where a central-cloud choice would shift the risk. The decision must be made before the model can be finalized (§8).
...
- **TB-5 — Site ↔ central offline sync.** Store-and-forward over the WAN, then replication. Append-only result versions with **explicit reconciliation — NO last-writer-wins**. Conflict and outage surface.
...
- **TB-7 — On-prem appliance / DB host ↔ physical world.** Edge boxes and per-site DB hosts can be stolen or imaged. Significance is set by **deployment topology (#3)**: on-prem-per-site multiplies this boundary across every lab.

**Topology note:** central-cloud + thin sites collapses TB-3/TB-7 into one hardened DC (smaller at-rest surface, but TB-5 becomes a hard online dependency and data-residency/NPC posture shifts). Full on-prem per site maximizes TB-7 (many boxes holding PHI) and TB-5 (many sync links) but tolerates WAN loss better. **Decision #3 must resolve before this model is final.**
...
| TB-5 sync | **T**ampering / D | **Sync conflict causes silent data loss / wrong result** — two sites edit the same result; last-writer-wins would silently drop one | A1 | **Append-only result versions + explicit reconciliation; NO last-writer-wins** (Stage 4); conflicting versions surfaced, never overwritten | **REQ-RES-02**, REQ-DATA-01 | L4, L5 |
| TB-5 sync | **D**enial of service | **WAN outage drops results** queued at a site | A1 | **Store-and-forward queue; zero loss across WAN outage**; replay on reconnect (Stage 4) | **REQ-RES-01** | L5 |
| TB-5 sync | **I**nfo disclosure / S | MITM or spoofed peer on the WAN sync link reads/forges PHI between site and central | A1, A5 | TLS on sync transport; mutual peer authentication; encrypted store-and-forward payloads; sync credential custody (REQ-SEC-05) | REQ-SEC-01, REQ-SEC-02, REQ-SEC-05 | L4, L6 |
...
| Store-and-forward; zero loss on WAN outage | REQ-RES-01 | **Stage 4** | Later |
| Append-only result versions + explicit reconciliation (no LWW) | REQ-RES-02 | **Stage 4** | Later |
| TLS on MLLP + sync (encryption in transit) | REQ-SEC-01 | **Stage 5** verified | Later |
| Encryption at rest | REQ-SEC-02 | **Stage 5** verified | Later |
...
- **R5 — Topology (#3) unresolved** leaves TB-3/TB-5/TB-7 risk un-finalized.
...
5. Sync conflict + WAN-outage chaos; confirm no silent loss / no LWW (REQ-RES-01/02) — overlaps L5.
...
- **Key & secrets management (REQ-SEC-05)** — where TLS private keys, at-rest encryption keys, and service/DB/sync credentials live (HSM? KMS? OS keystore? per-site vs central), rotation, and custody. Blocks the root-of-trust for REQ-SEC-01/02 (A7, R2). **[NEEDS-HUMAN]**
...
- **Deployment topology (open decision #3)** — central-cloud + thin sites vs. full on-prem per site + central sync. Materially changes TB-3/TB-5/TB-7, the at-rest surface, and the NPC data-residency posture. The model cannot be finalized until this is set. **[NEEDS-HUMAN]**

...
| **Edge / drivers** — analyzer interface engine (`openelis-analyzer-bridge`): per-transport listeners (MLLP/serial/ASTM/file), parse + LOINC/UCUM normalization, FHIR northbound (ADR-0015) | `edge/drivers/` (submodule, pin `10ec333` = release `3.0.8` w/ LIS-88 FILE→common pipeline + `rejected_bundles` DLQ of record, LIS-109 H99S blank suppression, LIS-28 registry-backed unit→UCUM, LIS-26 EC90 ASTM, EDAN H90 profile, LIS-95 QC/cal gate, branch `develop`) | `aiLabSolution/openelis-analyzer-bridge` (standalone export, not a fork) | [`contexts/edge-drivers/CONTEXT.md`](contexts/edge-drivers/CONTEXT.md) |

...
| `05-offline-sync-topology.excalidraw` | The **post-pilot M3 spoke** (ADR-0002): site (validated M1 base) ↔ LabSolution's own on-prem datacenter in PH (not public cloud); store-and-forward, append-only result versions, explicit reconciliation; gated by the compliance extra-work — not in the pilot |
